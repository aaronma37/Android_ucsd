...
//"gl_FragColor = vColor;" +
    ...
    public textclass(final Context activityContext, String string)
    {
        ...
        final float[] cubeTextureCoordinateData = new float[312];

        for (int i =0;i<cubeTextureCoordinateData.length;i++){
            if (i%8==1) {
                cubeTextureCoordinateData[i] =(float)(Math.floor(i/8)/40.5f);
            }
            else if(i%8==0) {
        ...
        //Load the texture
        mTextureDataHandle = loadTexture(mActivityContext, R.drawable.textsprites);
        mTextureDataHandleundercase = loadTexture(mActivityContext, R.drawable.textspritesunder);
    }
